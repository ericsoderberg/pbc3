...
class Bible
  BOOKS = [
    ...
    ["Judges"],
    ["Ruth"],
    ["1 Samuel", "1samuel", "1st samuel"],
    ["2 Samuel", "2samuel","2nd samuel"],
    ["1 Kings", "1kings", "1st kings"],
    ["2 Kings", "2kings", "2nd kings"],
    ["1 Chronicles", "1chronicles", "1st chronicles"],
    ["2 Chronicles", "2chronicles", "2nd chronicles"],
    ["Ezra"],
    ["Nehemiah"],
    ["Esther"],
    ["Job"],
    ...
    ["Acts"],
    ["Romans"],
    ["1 Corinthians", "1corinthians", "1st corinthians"],
    ["2 Corinthians", "2corinthians", "2nd corinthians"],
    ["Galatians"],
    ["Ephesians"],
    ["Philippians"],
    ["Colossians"],
    ["1 Thessalonians", "1thessalonians", "1st thessalonians"],
    ["2 Thessalonians", "2thessalonians", "2nd thessalonians"],
    ["1 Timothy", "1timothy", "1st timothy"],
    ["2 Timothy", "2timothy", "2nd timothy"],
    ["Titus"],
    ["Philemon"],
    ["Hebrews"],
    ["James"],
    ["1 Peter", "1peter", "1st peter"],
    ["2 Peter", "2peter", "2nd peter"],
    ["1 John", "1john", "1st john"],
    ["2 John", "2john", "2nd john"],
    ["3 John", "3john", "3rd john"],
    ["Jude"],
    ["Revelation"]
  ]
  
  def self.verse_ranges(string=nil, strict=false)
    VerseParser.new(string, strict).ranges
  end
  
  def self.structure
    books = BOOKS.map{|b| b[0]}
    structure = [{name: 'Old Testament', sections: []},
      {name: 'New Testament', sections: []}]
    testament = structure[0]
    section = nil
    books.each do |book|
      if 'Genesis' == book
  ...
  def self.matches(text)
    ...
    while index < terms.length
      ...
      # check for things like: "1 john" or "1st john"
      if term.match(/^1$|^2$|^3$|^1s|^2n|^3r/i) and next_term
        term += ' ' + next_term
        index += 1
        next_term = terms[index+1]
        #score += 1
      ...
      # check if this matches any books
      exp = /^#{term}/i
      books = BOOKS.select{|names| names.any? {|n| n.match(exp)}}
      if not books.empty?
        score += 1 if books.length == 1 and term.length > 2
      
        # check for chapter or chapter:verse
        if next_term and next_term.match(/^\d{1,3}$|^\d{1,3}:\d+/)
          matches = books.map{|names| names[0] + ' ' + next_term}
          term += ' ' + next_term
          index += 1
          next_term = terms[index+1]
        else
          matches = books.map{|names| names[0]}
        end
        
        result = {type: 'reference', text: term, matches: matches, score: score}
      end
